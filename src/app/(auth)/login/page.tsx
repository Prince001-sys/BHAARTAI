...
function LoginForm() {
  ...
  // Call our backend bridge to create/find the user in Supabase and get a session token
  const establishSession = async (idToken: string) => {
    ...
    // Set cookie on client side as well to guarantee it persists
    if (data.customToken) {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Hard redirect after cookie is confirmed set
      window.location.replace('/dashboard')
    } catch (err: any) {
      ...
      if (err.code === 'auth/user-not-found' || err.code === 'auth/wrong-password' || err.code === 'auth/invalid-credential') {
        ...
        errorMsg = 'Email is already in use. Try signing in instead.'
      } else if (err.message) {
